  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col animate-slide-in-right">
        {/*Header*/}
        <div className="flex items-center justify-between p-5 border-border-app-border">
          ...
          {items.length === 0 ? (
            ...
            items.map((item)=>(
              <div key={item.product._id} className="flex gap-3 bg-app-cream/60 rounded-xl p-3">
                ...
                className="size-16 rounded-lg object-cover shirnk-0" />
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-app-border space-y-3">
            ...
            flex-center gap-2 active:scale-[0.98]">Proceed to Checkout<ArrowRightIcon className="size-4" /></button>
          </div>
        )}
      </div>
    </>
